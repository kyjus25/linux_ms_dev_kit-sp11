...
/*
 * IPTS compatibility shim.  A second hidraw device with a fabricated, minimal
 * IPTS touchscreen descriptor receives complete HEAT cycles serialized into
 * IPTS DFT metadata/window reports, so the stock, unmodified iptsd can parse
 * the G6 digitizer through its ordinary hidraw path.  The contact thresholds
 * derive from the P4-P8 Windows capture corpus; see ADR0059/ADR0060.
 */
...
struct g6ts_ipts_banks {
	u8 count;
	struct g6ts_ipts_vector x[G6TS_IPTS_MAX_VECTORS];
	struct g6ts_ipts_vector y[G6TS_IPTS_MAX_VECTORS];
};

struct g6ts_ipts_part {
	bool present;
	u16 len;
	u8 content[G6TS_HEAT_MAX_CONTENT_SIZE];
};
...
struct g6ts_ipts {
	...
	/* Cycle scratch. */
	struct g6ts_ipts_banks position;
	struct g6ts_ipts_banks pressure;
	u8 detection[16];
	bool has_position;
	bool has_pressure;
	bool has_detection;
...
static void g6ts_ipts_parse_vector(struct g6ts_ipts_vector *v,
				   const u8 *entry)
{
	u8 i;
...
static void g6ts_ipts_sort_vectors(struct g6ts_ipts_vector *v, u8 n)
{
	u8 i;

	for (i = 1; i < n; i++) {
		struct g6ts_ipts_vector key = v[i];
		s8 j = i - 1;
...
static bool g6ts_ipts_section(const struct g6ts_ipts_part *part, size_t *end)
{
	u32 section_length;
 ...
 */
static void g6ts_ipts_scan_position(struct g6ts_ipts *ip)
{
	const struct g6ts_ipts_part *part = &ip->parts[0];
	size_t end, pos;
 ...
 */
static void g6ts_ipts_scan_pressure(struct g6ts_ipts *ip)
{
	const struct g6ts_ipts_part *part = &ip->parts[1];
	size_t end, pos;
...
static size_t g6ts_ipts_append_window(u8 *out, size_t at, u8 dft_type,
				      u32 ts_ms,
				      const struct g6ts_ipts_banks *banks,
...
static void g6ts_ipts_emit_cycle(struct g6ts *ts)
{
	 ...
	 */
	if (ip->has_pressure) {
		const struct g6ts_ipts_banks *p = &ip->pressure;
		u8 i;
 ...
 */
static void g6ts_ipts_emit_lift(struct g6ts *ts)
{
	struct g6ts_ipts *ip = ts->ipts;
	size_t at = 0, frame_at;
	u32 ts_ms;
...
static void g6ts_ipts_clear_cycle(struct g6ts_ipts *ip, bool incomplete)
{
	u8 i;
...
static void g6ts_ipts_process_cycle(struct g6ts *ts)
{
	struct g6ts_ipts *ip = ts->ipts;
 ...
 */
static void g6ts_ipts_feed(struct g6ts *ts, u8 report_id,
			   const u8 *content, u16 content_len)
{
	struct g6ts_ipts *ip = ts->ipts;
	s8 index = -1;
	...
	g6ts_ipts_process_cycle(ts);
	g6ts_ipts_clear_cycle(ip, false);
}

static void g6ts_ipts_boundary(struct g6ts *ts, u8 flags)
{
	struct g6ts_ipts *ip = ts->ipts;
...
static int g6ts_ipts_hid_raw_request(struct hid_device *hdev,
				     ...
				     int reqtype)
{
	struct g6ts_ipts *ip = hid_get_drvdata(hdev);
...
static void g6ts_ipts_unregister(struct g6ts *ts)
{
	struct g6ts_ipts *ip = ts->ipts;
	struct hid_device *hid;
	...
	if (!ip || !ip->hid)
		return;
	/*
	 * Injections run under io_lock, so retiring the pointer under the
	 * same lock guarantees no injection is in flight across teardown.
	 */
	mutex_lock(&ts->io_lock);
	hid = ip->hid;
	ip->hid = NULL;
...
static void g6ts_ipts_unregister_action(void *data)
{
	g6ts_ipts_unregister(data);
}

static int g6ts_ipts_register(struct g6ts *ts)
{
	struct hid_device *hid;
	int ret;
...
static ssize_t ipts_report_descriptor_read(struct file *filp,
					   struct kobject *kobj,
...
static BIN_ATTR_RO(ipts_report_descriptor,
		   sizeof(g6ts_ipts_report_descriptor));
...
static int g6ts_hid_parse(struct hid_device *hdev)
{
	...
	ret = hid_parse_report(hdev, ts->report_descriptor,
...
static const struct bin_attribute * const g6ts_bin_attributes[] = {
	...
	&bin_attr_ipts_report_descriptor,
	NULL,
};
...
static void g6ts_handle_data_report(struct g6ts *ts)
{
	...
	if (g6ts_is_raw_export_report(ts->last_content_id)) {
		g6ts_heat_enqueue(ts, ts->last_content_id, payload,
				  ts->last_content_len);
		/* Feed complete HEAT cycles to the IPTS compatibility shim. */
		g6ts_ipts_feed(ts, ts->last_content_id, payload,
			       ts->last_content_len);
	}
...
static int g6ts_probe(struct spi_device *spi)
{
	...
	if (g6ts_ipts_shim) {
		ts->ipts = devm_kzalloc(&spi->dev, sizeof(*ts->ipts),
					GFP_KERNEL);
		if (!ts->ipts)
			return -ENOMEM;
		ts->ipts->ts = ts;
		ts->ipts->inject_buf =
			devm_kmalloc(&spi->dev, G6TS_IPTS_REPORT_LEN,
				     GFP_KERNEL);
...
static void g6ts_remove(struct spi_device *spi)
{
	...
	if (ts->ipts) {
		cancel_delayed_work_sync(&ts->ipts->stale_work);
		g6ts_ipts_unregister(ts);
	}
