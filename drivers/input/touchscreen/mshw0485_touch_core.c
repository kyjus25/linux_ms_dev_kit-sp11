...
/*
 * Microsoft Surface G6 Touch (MSHW0485) transport and input driver.
 */
...
struct g6ts {
	...
	struct mshw0485_denali *denali;
	struct hid_device *hid;
	bool hid_ready;
	u8 *hid_inject_buf;
	struct input_dev *pen_input;
	bool native_pen_enabled;
...
static void g6ts_heat_generation_boundary(struct g6ts *ts, u8 flags)
{
	...
	mshw0485_denali_boundary(ts->denali);
	if (!heat)
		return;
...
static const char *g6ts_profile_name(void)
{
	...
	if (mshw0485_denali_ready_quiesce() && g6ts_feature70_one_byte)
		return "phase82";
	if (mshw0485_denali_ready_quiesce())
		return "phase81";
	if (mshw0485_denali_host_fault_recovery())
		return "phase80";
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return mshw0485_denali_format_stats(ts->denali, buf);
}
...
static ssize_t ipts_report_descriptor_read(struct file *filp,
					   ...
					   size_t count)
{
	return mshw0485_denali_read_report_descriptor(buf, off, count);
}
...
static void g6ts_handle_data_report(struct g6ts *ts)
{
	...
	if (g6ts_is_raw_export_report(ts->last_content_id)) {
		...
		mshw0485_denali_feed(ts->denali, ts->last_content_id,
				       payload, ts->last_content_len);
	}
					 ...
					 enum g6ts_recovery_path path)
{
	...
	if (ts->denali && !mshw0485_denali_ready(ts->denali)) {
		ret = mshw0485_denali_register(ts->denali, ts->product_id);
		if (ret)
...
static int g6ts_probe(struct spi_device *spi)
{
	...
	if (mshw0485_denali_ready_quiesce() && !mshw0485_denali_host_fault_recovery())
		return dev_err_probe(&spi->dev, -EINVAL,
	...
	if (!ts->hid_inject_buf)
		return -ENOMEM;
	ts->spi = spi;
	ts->native_pen_enabled = !mshw0485_denali_enabled();
	/*
	...
	if (mshw0485_denali_enabled()) {
		ts->denali = mshw0485_denali_create(&spi->dev);
		if (IS_ERR(ts->denali))
			return PTR_ERR(ts->denali);
		ret = devm_add_action_or_reset(&spi->dev,
					       mshw0485_denali_destroy_action,
					       ts->denali);
		if (ret)
			return ret;
	}
...
static void g6ts_remove(struct spi_device *spi)
{
	...
	if (ts->denali) {
		devm_remove_action(&spi->dev,
				   mshw0485_denali_destroy_action,
				   ts->denali);
		mshw0485_denali_destroy(ts->denali);
	}
